...
- **Custom components receive `playbackRate` again**: `AudioPlayer.CustomComponent` children read player state from an `audioPlayerState.curAudioState` object that is rebuilt from the split contexts. `playbackRate` was never added to that rebuild when playback speed support shipped, so it always arrived as `undefined`. The rebuild now forwards the whole playback slice, so speed reads correctly and later playback fields cannot silently go missing.
