...
Each `<AudioPlayer>` instance gets a unique id of the form
`rm-audio-player-{autoGeneratedId}` (and `rm-audio-player-audio-{autoGeneratedId}`
for the internal `<audio>` element). The suffix is generated with React's
`useId()` so multiple players on the same page do not collide.

Because the suffix changes per instance, the id is **not** a stable CSS
selector — use the root **className** below for styling.

### root ClassName

- `rmap-player-provider` — provider container (theme, CSS variables)
- `rmap-player-container` — player root (inside the provider)
